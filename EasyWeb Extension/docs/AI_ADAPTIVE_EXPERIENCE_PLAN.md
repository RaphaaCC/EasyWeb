...
> **Status:** implementação beta em andamento na versão `v1.0.2 Beta`. O nível 1 já possui contrato WebSocket, armazenamento de planos base, integração com Gemini, validação de um catálogo fechado de ações, compilação local de CSS e cache local. Os recursos de níveis 2 e 3 continuam somente planejados.
>
