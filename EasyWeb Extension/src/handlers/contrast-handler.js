  ...
  function create(getTextElements) {
    ...
    function apply(active) {
      ...
      for (const element of getTextElements()) {
        const adjustment = chooseColor(element);
        if (adjustment) {
      ...
      for (const element of document.querySelectorAll(
        ...
        if (!hasVisibleBorder) {
          element.setAttribute("data-easyweb-control-outline", candidates[0].name);
        }
        if (adjustment || !hasVisibleBorder) {
    ...
    function getCachedCssRules() {
      ...
      for (const element of adjustedControls) {
        ...
        if (outline) {
          const outlineColor = outline === "dark" ? "#000" : "#fff";
          rules.push(`${prefix} ${selector}{outline:2px solid ${outlineColor}!important;outline-offset:-2px!important;}`);
        }
      }
